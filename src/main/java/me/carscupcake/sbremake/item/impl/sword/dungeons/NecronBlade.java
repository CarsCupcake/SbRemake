...
public class NecronBlade implements ISbItem, ISbItem.StatProvider, Dungeonizable, IContextAbilities, Listener {
    ...
    public static final ItemAbility<PlayerInteractEvent> IMPLOSION = new ItemAbility<>("Implosion", AbilityType.RIGHT_CLICK, event -> {
        ParticleUtils.spawnParticle(event.player().getInstance(), event.player().getPosition().add(0, event.player().getEyeHeight() / 2, 0), Particle.EXPLOSION,
                6, Vec.ZERO, 10);
        ...
        var total = event.player().getInstance().getNearbyEntities(event.player().getPosition(), 6).stream()
                .filter(entity -> entity instanceof SkyblockEntity)
                .map(entity -> {
                    totalEntities.set(totalEntities.get() + 1);
                    return (SkyblockEntity) entity;
                })
                .map(entity -> entity.mageDamage(event.player(), intelligence, 10_000, 0.3))
        ...
        if (totalEntities.get() != 0)
            ...
            new Lore("§7Deals §c%d% §7to nearby enemies.", "%d%", new Lore.AbilityDamagePlaceholder(10_000, 0.3)),
            new ManaRequirement<>(300), new CooldownRequirement<>(10));

    public static final ItemAbility<PlayerInteractEvent> SHADOW_WARP = new ItemAbility<>("Shadow Warp", AbilityType.RIGHT_CLICK, event -> {

    }, new Lore("§7Created a spacial distortion §e10§7 blocks ahead of you that sucks all enemies around it. Using this ability again with §e5§7 seconds to detonate the warp and deal §c%d%§7 damage to enemies near it.",
            "%d%", new Lore.AbilityDamagePlaceholder(10_000, 0.3)),
            new ManaRequirement<>(300), new CooldownRequirement<>(10));
    ...
    public static final ItemAbility<PlayerInteractEvent> WITHER_SHIELD = new ItemAbility<>("Wither Shield", AbilityType.RIGHT_CLICK, event -> {
        ...
        new TaskScheduler() {
            ...
            public void run() {
                ...
                ParticleUtils.spawnParticle(event.player().getInstance(), event.player().getPosition().add(0, event.player().getEyeHeight() / 2, 0),
                        Particle.HEART,
                        5, new Vec(0.5, 0.5, 0.5), 1);
            }
        }.delayTask(20 * 5);
    }, new Lore("§7Reduces damage taken by §c10%§7 for §e5§7 seconds. Also grants an §6❤ Absorption§7 shield that gives §9150%§7 of your " + Stat.CritDamage + "§7 as SkyBlock " + Stat.Health + "§7, after §e5§7 seconds §a50%§7 of the shield is converted into healing.")
            , new ManaRequirement<>(150), new CooldownRequirement<>(10));
    ...
    public static final ItemAbility<PlayerInteractEvent> WITHER_IMPACT = new ItemAbility<>("Wither Impact", AbilityType.RIGHT_CLICK, event -> {
        ...
        if (!event.player().getInstance().getBlock(event.getPlayer().getPosition().add(0, 2, 0)).isSolid()) {
            var pos = event.player().getPosition().add(0, 0.5, 0);
    ...
    }, new Lore("§7Teleport §a10 blocks§7 ahead of you. Then implode dealing §c%d% " + Stat.Damage + "§7 to nearby enemies. Also applies the wither shield scroll ability reducing damage taken and granting an §6❤ Absorption§7 shield for §e5§7 seconds.",
            "%d%", new Lore.AbilityDamagePlaceholder(10_000, 0.3)), new ManaRequirement<>(300), new CooldownRequirement<>(0.15));
    ...
    public static final Modifier<List<Ability>> WITHER_SCROLLS = new Modifier<>() {
        ...
        public SbItemStack toNbt(List<Ability> abilities, SbItemStack itemStack) {
            ...
            if (abilities.size() == 3) {
                ...
                return SbItemStack.from(itemStack.item().withTag(EXTRA_ATTRIBUTES,
                        ((CompoundBinaryTag) itemStack.item().getTag(EXTRA_ATTRIBUTES)).put("ability_scrolls", scrolls.build())));
            }
            for (var ability : abilities) {
                ...
                if (ability == SHADOW_WARP)
                    scrolls.add(StringBinaryTag.stringBinaryTag("SHADOW_WARP_SCROLL"));
                if (ability == WITHER_SHIELD)
                    scrolls.add(StringBinaryTag.stringBinaryTag("WITHER_SHIELD_SCROLL"));
                if (ability == IMPLOSION)
                    scrolls.add(StringBinaryTag.stringBinaryTag("IMPLOSION_SCROLL"));
            }
            return SbItemStack.from(itemStack.item().withTag(EXTRA_ATTRIBUTES,
                    ((CompoundBinaryTag) itemStack.item().getTag(EXTRA_ATTRIBUTES)).put("ability_scrolls", scrolls.build())));
    ...
    public EventNode<Event> node() {
        return EventNode.all("wither_scrolls.ability").addListener(EntityMeleeDamagePlayerEvent.class, event -> {
            if (witherShields.contains(event.getPlayer()))
                event.setNormalDamage(event.getNormalDamage() * 0.9);
        })
                .addListener(GetItemStatEvent.class, event -> {
                    ...
                    switch (event.getItemStack().sbItem()) {
                        case Hyperion _ -> {
                            if (event.getStat() == Stat.Intelligence)
                                event.addValue(catalevel * 2);
                        }
                        case Valkyrie _ -> {
                            if (event.getStat() == Stat.Strength)
                                event.addValue(catalevel);
                        }
                        case Scylla _ -> {
