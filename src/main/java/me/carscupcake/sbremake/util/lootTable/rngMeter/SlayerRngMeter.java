...
public class SlayerRngMeter {
    ...
    public SlayerRngMeter(SkyblockPlayer player, ISlayer slayer, Map<RngMeterEntry, Double> lootTableGoal, Map<RngMeterEntry, Double> lootTableChances) {
        ...
        String selected = file.get(STR."\{slayer.getId()}_SELECTED", ConfigSection.STRING);
        if (selected != null) {
    ...
    public void save() {
        ...
        file.set(STR."\{slayer.getId()}_SELECTED", selected == null ? null : selected.id(), ConfigSection.STRING);
        file.save();
    }
