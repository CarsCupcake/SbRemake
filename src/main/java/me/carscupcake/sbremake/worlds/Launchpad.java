...
public record Launchpad(int x1, int z1, int x2, int z2, int y, SkyblockWorld targetWorld, Pos targetPos) {
    public void launch(SkyblockPlayer player) {
        ...
        WorldProvider provider = SkyblockWorld.getBestWorld(player, targetWorld);
        Future<WorldProvider> instanceContainerFuture;
        if (provider == null) {
        ...
        new TaskScheduler() {
            ...
            public void run() {
                if (i == iterations) {
                    cancel();
                    try {
                        WorldProvider finalProvider = instanceContainerFuture.get();
                        synchronized (player) {
