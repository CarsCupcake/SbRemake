    ...
    public void openStatBreakdown(Stat stat, SkyblockPlayer player) {
        ...
        for (PlayerStatEvent.PlayerStatModifier modifier : event.modifiers()) {
            switch (modifier.type()) {
                case Value -> flatBonuses.addLoreRow( (stat.getPrefix()) + "+" + (modifier.value()) +  (stat.getSymbol()) + "§7 " + (modifier.name()) );
                case AddativeMultiplier -> additiveBonuses.addLoreRow( (stat.getPrefix()) + "+" + (modifier.value()) + "%§7 " + (modifier.name()) );
                case MultiplicativeMultiplier -> multiplicativeBonuses.addLoreRow( (stat.getPrefix()) + "+" + (modifier.value()) + "x§7 " + (modifier.name()) );
            }
        }
