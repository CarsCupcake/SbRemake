...
public class MiningSkill extends ISkill {
    ...
    public MiningSkill(SkyblockPlayer player) {
        super(player, "mining", Material.STONE_PICKAXE, Material.IRON_BLOCK,
                new Lore(List.of("§7Dive into deep caves and find rare", "§7ores and valuable materials to earn", "§7Mining XP!", "§7 ",
                        "§eSpelunker %level%", STR." §fGrants §a%bonus%%  \{Stat.MiningFortune}§f, which", " §fincreases your chance for", " §fmultiple ore drops"), Map.of(
    ...
    public record SpelunkerReward(int level) implements SkillReward {
        ...
        @Override
        public Lore lore() {
            return new Lore(List.of(STR."§eSpelunker \{StringUtils.toRoman(level)}", STR." §fGrants \{(level == 0) ? "" : STR."§8\{4 * (level - 1)}➜"}§a\{4 * level} \{Stat.MiningFortune}§f, which", " §fincreases your chance for", " §fmultiple ore drops", STR."§8+ §a\{(level < 15) ? 1 : 2} \{Stat.Defense}"));
        }
    }
}
